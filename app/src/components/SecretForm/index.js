...
const MAIN_INFO_FORM_CONFIGS = {
    ...
    password: {
        ...
        name: '密码',
        required: true,
        validators: [Validators.required("密码不能为空"), (value) => value?.length < 6 && '密码长度小于6'],
        mapEventToValue: MapFieldChangeEventToValue,
        buttonSlot: (onClick) => (
            <View slot="button">
                <Button 
...
export default function({secret, onSubmit}) {
    ...
    const socialList = useMemo(() => {
        ...
        const handleDeleteItem = (socialItem) => {
            ...
            const index = socialList.findIndex(item => item.get('_id') === socialItem.get('_id'));
